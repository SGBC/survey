...
pigw21 <- read_excel("pigw21s.xlsx")
...
pigw21Pile = cbind(pigw21Done, pigw21Q1)
...
sod21Q1$Q1 = dplyr::recode(sod21Q1$Q1, "Within the last year" = 1, "1-2 years ago" = 2, "3-5 years ago" = 3, "More than 5 years ago" = 4, "I have never submitted a manuscript to a journal" = 5)
...
ggplot() +
     geom_step(aes(x = as.numeric(Q1)), data = sod21Q1, stat = "ecdf") + 
     geom_step(aes(x = as.numeric(Q1)), data = pigw21Q1, stat = "ecdf", color="#c78b90") +
...
ks.test(sod21Q1$Q1, pigw21Q1$Q, ifresult = FALSE) #Kolmogorov-Smirnov test for Q1
```



```{r question_2, echo=T, eval = F, message=FALSE, warning=FALSE}
...
pigw21Pile = cbind(pigw21Done, pigw21Q2)
pigw21 = dplyr::select(pigw21,!1)

Q2 = 'When was the last occasion that you published or submitted a manuscript to a journal?'
colnames(sod21Q2) = c('Q2')
colnames(pigw21Q2) = c('Q2')

#Necessary harmonisation of response formats, in this case we make sure it is numeric and remove rows with 0 in year of first publication.
pigw21Q2$Q2=as.numeric(pigw21Q2$Q2)
pigw21Q2$Q2 = subset(pigw21Q2$Q2, !Q2==0)


#This would be the function where we pass sod21Q2, pigw21Q2, the column selection (ie named Q2 here) and the Xlab.
ggplot() +
     geom_step(aes(x = as.numeric(Q2)), data = sod21Q2, stat = "ecdf") + 
     geom_step(aes(x = as.numeric(Q2)), data = pigw21Q2, stat = "ecdf", color="#c78b90") +
  scale_x_continuous(limits = c(1990, 2022)) +
  xlab("Year of first publication by respondent")

ks.test(sod21Q2$Q2, pigw21Q2$Q2, ifresult = FALSE) #Kolmogorov-Smirnov test for Q2
```

```{r question_3, echo=T, eval = F, message=FALSE, warning=FALSE}
